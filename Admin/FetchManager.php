...
if(isset($_POST["query"]))
{
 ...
 $query = "
 SELECT manager.FirstName,manager.LastName,manager.ID AS managID, department.Name FROM manager
  INNER JOIN department ON department.ID =manager.DepID
  WHERE
  (manager.ID !='".$_SESSION['Manager_ID']."')AND
...
{
 $query = "
 SELECT manager.FirstName,manager.LastName,manager.ID AS managID, department.Name  FROM manager INNER JOIN department ON department.ID =manager.DepID
...
if(mysqli_num_rows($result) > 0)
{
 ...
 while($row = mysqli_fetch_array($result))
 {
    ...
    <td> <strong id="id'.$row["managID"].'">'.$row["Name"].'</strong></td>
    ...
    <td>
      ...
      <div class="dropdown">
        ...
        <div class="dropdown-menu">
          <form method="get" action="ManagProfile.php">
        <input type="hidden" id="managIDedit" name="managIDedit" value="'.$row["managID"].'"/>
          ...
          echo"'";
          echo $row["managID"]."','".$row["FirstName"]."','".$row["LastName"]."'";
          echo ')"'; 
  ...
  function DeleteEmp(id,first,last){
      ...
      document.getElementById("managID").value=id;
