...
if (isset($_GET['empIDedit'])){
    $result = mysqli_query($connection, "SELECT employee.firstName,employee.LastName,employee.ID,employee.Pic ,employee.DepID,
     department.Name AS DepName, branch.Name AS BranName
     from employee INNER JOIN department ON department.ID=employee.DepID
                   INNER JOIN branch ON department.BranchID=branch.ID

      WHERE employee.ID=" . $_GET['empIDedit'] );
...
if (isset($_POST['save'])) {
    ...
    if(isset($fileName))
    {
        if(!empty($fileName))
        {
            ...
            if(move_uploaded_file($tempName, $location.$empIDS.'.'.$extension))
            {


              $query="UPDATE `employee` SET `ID` = '".$empIDS."',firstName = '".$firstnameS."',
              LastName='".$lastnameS."',DepID='".$departmentS."',Pic='".$extension."'
               WHERE `employee`.`ID` = '".$empID."';";
                  
              
                  if(mysqli_query($connection,$query)){
                      echo '<script type="text/javascript">'; 
                      echo 'alert("Employee information updated successfully ✓ ");'; 
                     ...
                     if(mysqli_errno($connection) == 1062){
                         echo '<script>alert("Duplicated ID ( '.$empIDS.' ) entred ")</script>';
                         $_POST = array();}
                     else{
...
$query="UPDATE `employee` SET `ID` = '".$empIDS."',firstName = '".$firstnameS."',
LastName='".$lastnameS."',DepID='".$departmentS."'
 WHERE `employee`.`ID` = '".$empID."';";
                  ...
                  <div class="nav-align-top mb-4">
                    ...
                    <div class="tab-content">
                      ...
                      <div class="d-flex align-items-start align-items-sm-center gap-4">
                        <img src="../assets/img/avatars/<?php echo$empID.'.'.$Pic;?>" alt="user-avatar" class="d-block rounded" height="100" width="100" id="uploadedAvatar">
                        ...
                        <div class="row mb-3">
                          ...
                          <div class="col-sm-10">
                            <div class="input-group input-group-merge">
                              ...
                              <input type="text" name="empID" id="empID" class="form-control"  required placeholder="<?php echo $empID;?>" value="<?php echo $empID;?>" >
                            </div>
                          </div>
                        </div>
                        <div class="row mb-3">
...
from department INNER JOIN branch ON branch.ID=department.BranchID
 ...
 ?>
              ...
              <!--/ Layout Demo --><div class="card">
                    <h5 class="card-header">Delete Employee</h5>
                    <div class="card-body">
                      <div class="mb-3 col-12 mb-0">
                        <div class="alert alert-warning">
                          <h6 class="alert-heading fw-bold mb-1">Are you sure you want to delete employee?</h6>
                          <p class="mb-0">Once you delete the employee, there is no going back. Please be certain.</p>
                        </div>
                      </div>
                      <form id="formAccountDeactivation" method="get" action="Delete.php" >
                        <div class="form-check mb-3">
                            <input type="hidden" name="EmpID" value="<?php echo $_GET['empIDedit'];?>"/>
 
                            <input class="form-check-input" type="checkbox" name="accountActivation" id="accountActivation">
                          <label class="form-check-label" for="accountActivation">I confirm  employee deletation</label>
                        </div>
                        <button type="submit" name="deleteBut" class="btn btn-danger deactivate-account">Delete Employee</button>
                      </form>
                    </div>
                  </div>
            </div>
