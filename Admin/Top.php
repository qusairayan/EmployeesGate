...
<?php include "../db/db.php";
 ...
 <!-- The Modal -->
   ...
   <div class="col-xl-6" >
    ...
    "><form action="admin.php"method="Post" style="height:auto"><select id="LastMonths"   onchange="this.form.submit()" class="form-select" name="period"
                     style="
    ...
    margin: -0.7rem  1rem;
    color: #fefefe;">
    <?php if (isset($_GET['DateTo'])){echo'<option selected disabled >Pleas Select Period</option>
';}?>
    ...
    align-items: center;
    flex-wrap: nowrap;" >              
            <label style="width:auto"for="html5-date-input" class="col-md-2 col-form-label">From</label>

               <input value="<?php if (isset($_GET['DateTo'])){echo$_GET['DateFrom'];}?>" name="DateFrom" id="DateFrom" style="width:auto; margin:0.5rem" class="form-control" type="date"  id="html5-date-input">
              <label id="DateToLabel"  style="<?php  if (!isset($_GET['DateTo'])){echo 'display:none;';}?> width:auto; margin:0.5rem" for="html5-date-input" class="col-md-2 col-form-label">to</label>

              <input value="<?php if (isset($_GET['DateTo'])){echo $_GET['DateTo'];}?>" 
               name="DateTo" id="DateTo" style=" width:auto; <?php  if (!isset($_GET['DateTo'])){echo 'display:none';}?>" class="form-control" type="date"  id="html5-date-input">
                       </div> 
                      </div></div>
     ...
     <div class="container" data-aos="fade-up">
       ...
       <div class="row justify-content-center" data-aos="fade-up" data-aos-delay="150">
         ...
         <div class="col-xl-6 col-lg-8">
           <h1 >Top Rated Heartsits at Aqaba City </h1>
...
LIMIT 1
";}}}
elseif (isset($_GET['DateTo'])){

  $sql1 = "SELECT *,FirstName,LastName,employee.Pic,employee.DepID,Name,SUM(rate.Rate) as total
  FROM `rate`
 INNER JOIN employee ON employee.ID=rate.EmpID 
 INNER JOIN department ON department.ID=rate.DepID 
 WHERE rate.DepID=".$row['ID']." AND ( date_time BETWEEN'". $_GET['DateFrom']."' AND '".$_GET['DateTo']."')
 GROUP by EmpID
 ORDER BY total DESC 
 LIMIT 1
 ";
...
$( "#DateFrom" ).change(function() {
  var z = document.getElementById("DateTo");
  var l = document.getElementById("DateToLabel");
  var from =document.getElementById("DateFrom").value;

z.style.display = "block";
l.style.display = "block";
if(z.value !=""){
  window.location.href = "admin.php?DateFrom="+from+"&DateTo="+z.value;
...
$( "#DateTo" ).change(function() {
  ...
  window.location.href = "admin.php?DateFrom="+from+"&DateTo="+to;
...
today = yyyy + '-' + mm + '-' + dd;
yesterday = yyyy + '-' + mm + '-' + ddd;

document.getElementById("DateTo").setAttribute("max", today);
document.getElementById("DateFrom").setAttribute("max", yesterday);
</script>