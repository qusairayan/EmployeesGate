...
if(isset($_POST["query"]))
{
 ...
 $query = "
 SELECT *, manager.ID as managerID FROM manager
 INNER JOIN department ON department.ID =manager.DepID
 WHERE (DepID LIKE '".$_SESSION['Department']."%' AND Position='Manager' ) 
  AND
...
else
{
 ...
 $query = "
 SELECT manager.FirstName,manager.LastName,manager.Pic,manager.ID AS managerID, department.Name, department.BranchID
   FROM manager
  INNER JOIN department ON department.ID = manager.DepID
  WHERE DepID LIKE '".$_SESSION['Department']."%' AND Position='Manager' 
...
if(mysqli_num_rows($result) > 0)
{
...

 {
  ...
  echo '>
  <span class="tf-icons bx bxs-message-alt-add"></span> تقييم
</button>
